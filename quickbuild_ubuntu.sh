...
if [ -f /mnt/archiv/LessLinux/llbuild/stage01.tar.xz ] ; then
	...
	gnome-terminal --hide-menubar -t "LOG: stage01" -e "tail -f /mnt/archiv/LessLinux/llbuild/stage01.log" 2> /dev/null &
	if [ "$arch" = x86_64 ] ; then
...
if [ -f /mnt/archiv/LessLinux/llbuild/stage01.tar.xz ] ; then
	...
	echo "Building stage02 - please be patient."
        ...
        gnome-terminal -t "LOG: stage02" -e "tail -f /mnt/archiv/LessLinux/llbuild/stage02.log" 2> /dev/null &
        if [ "$arch" = x86_64 ] ; then
