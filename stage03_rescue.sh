...
if [ "$arch" = x86_64 ] ; then
linux32 ruby -I. builder.rb -u -n -s 1,2,bootconf \
-p config/pkglist_neutral_rescue_GTK3.txt \
-c config/general_neutral_rescue.xml \
-b config/branding_neutral_rescue.xml \
-k config/kernels_rescue_experimental.xml \
--skip-files config/skiplist_neutral_rescue.txt > /mnt/archiv/LessLinux/llbuild/stage03.log 2>&1
else
ruby -I. builder.rb -u -n -s 1,2,bootconf \
-p config/pkglist_neutral_rescue_GTK3.txt \
-c config/general_neutral_rescue.xml \
-b config/branding_neutral_rescue.xml \
-k config/kernels_rescue_experimental.xml \
